...
def run_processor(file_entry: tk.Entry, attr_entry: tk.Entry, value_entry: tk.Entry, output_entry: tk.Entry) -> None:
    ...
    if not filepath or not attr or not value:
        messagebox.showerror("Ошибка", "Заполните все поля!")
        return

    try:
        processor = ExcelProcessor(filepath=filepath, field_name=attr, value=value, output_path=output_file)
...
def create_gui():
    ...
    # --- атрибут ---
    tk.Label(root, text="Атрибут:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
    attr_entry = tk.Entry(root, width=40)
    attr_entry.grid(row=2, column=1, padx=5, pady=5, columnspan=2)

    # --- значение ---
    tk.Label(root, text="Значение:").grid(row=3, column=0, padx=5, pady=5, sticky="w")
    value_entry = tk.Entry(root, width=40)
    value_entry.grid(row=3, column=1, padx=5, pady=5, columnspan=2)

    # --- кнопка запуска ---
    tk.Button(root, text="Запустить", command=lambda: run_processor(file_entry, attr_entry, value_entry, output_file), bg="lightgreen").grid(
